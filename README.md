...
5. 아래 표대로 **속성(Property) 3개**를 만듭니다. **이름과 종류가 정확히 같아야 합니다.**
   ...
   | `참석여부` | 선택 (Select) | 옵션 3개를 미리 만들어 두세요 → **`참석`**, **`불참`**, **`미정`** |
   ...
   > 🔤 속성 이름을 꼭 한글로 안 해도 됩니다. 다르게 지었다면 7단계에서
   > `NOTION_PROP_NAME` / `NOTION_PROP_STATUS` / `NOTION_PROP_MESSAGE` 환경변수로 알려주면 됩니다.
...
{
  "ok": true,
  "counts": { "total": 20, "참석": 12, "불참": 3, "미정": 5 },
  "attendees": [
    { "name": "김한빛", "status": "참석", "message": "보고싶다!", "updatedAt": "2026-09-01T10:00:00.000Z" }
  ]
}
```
...
```json
{ "name": "김한빛", "status": "참석", "message": "늦게라도 갈게요" }
```

- `status` 는 `참석` · `불참` · `미정` 중 하나여야 합니다.
...
```json
{ "ok": true, "updated": true, "attendee": { "name": "김한빛", "status": "참석", "message": "늦게라도 갈게요" } }
```
